...
export default function WarrantyPage() {
  return (
    <div className="min-h-screen bg-slate-50 pt-56 pb-24">
        ...
        <div className="space-y-20">
          ...
          <div className="space-y-10">
            <div className="flex items-center gap-6">
                ...
                <div className="w-12 h-12 rounded-[1rem] bg-black/[0.03] border border-black/10 flex items-center justify-center text-slate-700">
                   <Terminal className="w-6 h-6" />
                </div>
            </div>
            <h1 className="text-7xl md:text-9xl font-black text-slate-900 tracking-tighter leading-[0.8] uppercase italic drop-shadow-[0_20px_50px_rgba(0,0,0,0.8)]">WARRANTY <br /><span className="text-primary italic">PROTOCOL</span></h1>
          ...
          <div className="p-12 rounded-[4rem] bg-black/[0.02] border border-black/5 backdrop-blur-3xl space-y-8 shadow-2xl relative overflow-hidden group">
...
function PolicySection({ title, content }: { title: string, content: string }) {
  return (
    <div className="p-12 rounded-[3.5rem] bg-black/[0.02] border border-black/5 space-y-6 hover:border-primary/50 transition-all duration-700 shadow-2xl group relative overflow-hidden">
      <div className="absolute top-0 right-0 p-8 opacity-20 group-hover:opacity-100 transition-opacity duration-700">
         <ShieldCheck className="w-8 h-8 text-primary" />
      </div>
      <h3 className="text-2xl font-black text-slate-900 uppercase tracking-tighter italic group-hover:text-primary transition-colors leading-none">{title}</h3>
      <p className="text-slate-500 font-black uppercase tracking-tight italic leading-relaxed text-lg group-hover:text-slate-600 transition-colors">{content}</p>
    </div>
  );
}
