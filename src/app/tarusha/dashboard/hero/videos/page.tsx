  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col gap-2">
        <h1 className="text-4xl font-extrabold tracking-tight text-slate-900">Hero Videos</h1>
        <p className="text-slate-600 font-medium">Manage cinematic video backgrounds for the homepage hero section.</p>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-8">
          ...
          <CardContent>
            <form action={addHeroVideo} className="space-y-6">
              <div className="space-y-2">
                <Label htmlFor="title" className="text-slate-700 font-bold text-[11px] uppercase tracking-wider">Title Overlay</Label>
                <Input 
                  id="title" 
                  name="title" 
              ...
              <div className="space-y-2">
                <Label htmlFor="subtitle" className="text-slate-700 font-bold text-[11px] uppercase tracking-wider">Subtitle</Label>
                <Textarea 
                  id="subtitle" 
                  name="subtitle" 
                  placeholder="e.g., Powered by NVIDIA RTX 4090" 
                  className="bg-black/5 border-black/10 text-slate-900 min-h-[100px]"
                />
              </div>

              <div className="space-y-2">
                <Label className="text-slate-700 font-bold text-[11px] uppercase tracking-wider">Video File</Label>
        ...
        <div className="lg:col-span-2 space-y-6">
          {videos?.map((vid) => (
            <Card key={vid.id} className="bg-white/40 border-black/5 backdrop-blur-md overflow-hidden group">
              <div className="grid md:grid-cols-2">
                <div className="aspect-video relative bg-black">
                  ...
                  <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <Play className="w-12 h-12 text-slate-900 fill-current" />
                  </div>
                </div>
                <div className="p-8 flex flex-col justify-between">
                  <div>
                    <h3 className="text-xl font-black text-slate-900 uppercase tracking-tighter mb-2">{vid.title || 'Untitled Video'}</h3>
                    <p className="text-slate-600 text-sm font-medium leading-relaxed">{vid.subtitle}</p>
                  </div>
          ...
          {!videos?.length && (
            <div className="py-40 glass rounded-[3rem] border border-dashed border-black/10 flex flex-col items-center justify-center text-slate-600 gap-4">
