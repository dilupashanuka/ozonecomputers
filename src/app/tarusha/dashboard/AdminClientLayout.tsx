...
export function AdminClientLayout({ user, children }: AdminClientLayoutProps) {
  ...
  return (
    <div className="flex min-h-screen bg-[#050811] text-slate-400 selection:bg-primary/30">
      ...
      {/* Sidebar */}
      <aside className={cn(
        "bg-[#080b11] border-r border-white/5 flex flex-col fixed md:sticky top-0 h-screen transition-all duration-700 z-[101]",
        "w-80 md:flex",
        ...
        <div className="p-10 border-b border-white/5 flex items-center justify-between relative overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-4 mb-2">
              ...
              <h2 className="text-2xl font-black tracking-tighter text-white italic">OZONE <span className="text-primary">CORE</span></h2>
            </div>
        ...
        <div className="px-8 py-6">
          <div className="flex items-center gap-4 p-4 rounded-2xl bg-white/[0.02] border border-white/5">
            <div className="w-12 h-12 rounded-2xl bg-slate-900 border border-white/10 flex items-center justify-center text-sm font-black text-primary italic">
              {user.email?.[0].toUpperCase()}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-xs font-black text-white truncate uppercase italic">{user.email?.split('@')[0]}</p>
              ...
              <div className="space-y-1.5">
                {group.items.map((item) => (
                  <Link 
                    key={item.href}
                    href={item.href} 
                    className="group flex items-center gap-4 px-5 py-3 text-[11px] font-black rounded-xl hover:bg-white/5 text-slate-500 hover:text-white transition-all duration-300 italic uppercase tracking-wider"
                    onClick={() => setIsSidebarOpen(false)}
                  >
        ...
        <div className="p-8 border-t border-white/5 bg-[#0a0d14]">
          <form action={logout}>
      ...
      <main className="flex-1 relative flex flex-col min-h-screen overflow-x-hidden">
        ...
        <header className="h-24 border-b border-white/5 flex items-center px-10 md:px-14 sticky top-0 bg-[#050811]/90 backdrop-blur-2xl z-[90]">
          <Button 
            variant="ghost" 
            size="icon" 
            className="md:hidden mr-6 text-white" 
          ...
          <div className="flex items-center gap-4 text-[10px] md:text-xs font-black text-slate-600 italic uppercase tracking-[0.2em]">
            ...
            <span className="text-slate-400 truncate max-w-[150px] md:max-w-none">Node Controller</span>
          </div>
