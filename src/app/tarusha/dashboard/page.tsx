...
export default async function AdminDashboard() {
  ...
  return (
    <div className="space-y-16 animate-in fade-in slide-in-from-bottom-12 duration-1000">
      {/* Header Section */}
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-10 border-b border-black/5 pb-14 relative">
        ...
        <div className="space-y-5 relative z-10">
          ...
          <h1 className="text-6xl md:text-7xl font-black tracking-tighter text-slate-900 uppercase italic leading-none">OZONE <span className="text-primary">CORE</span> TERMINAL</h1>
        ...
        <div className="flex flex-wrap items-center gap-4 relative z-10">
          <Link href="/tarusha/dashboard/products/new">
            <Button className="bg-primary hover:bg-primary/90 text-slate-900 font-black h-20 px-10 rounded-[2rem] shadow-[0_20px_50px_rgba(239,68,68,0.3)] uppercase tracking-[0.2em] text-xs transform transition-all hover:-translate-y-2 italic gap-4">
              <Plus className="w-6 h-6" /> INJECT COMPONENT
            </Button>
          </Link>
          <Link href="/" target="_blank">
            <Button variant="outline" className="border-black/10 text-slate-500 h-20 px-10 rounded-[2rem] bg-black/[0.03] hover:bg-white/[0.08] hover:text-slate-900 uppercase tracking-[0.2em] text-xs font-black transition-all italic gap-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
        {stats.map((stat, i) => (
          <Link key={i} href={stat.link}>
            <Card className="bg-black/[0.02] border-black/10 backdrop-blur-3xl hover:border-primary/50 transition-all duration-700 group cursor-pointer rounded-[3rem] overflow-hidden relative">
              ...
              <CardContent className="p-12 relative z-10">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex items-center gap-3 text-[9px] font-black text-slate-600 uppercase tracking-[0.3em] bg-black/5 px-5 py-2 rounded-full italic border border-black/5">
                ...
                <div className="mt-10 space-y-3">
                  ...
                  <h3 className="text-7xl font-black text-slate-900 tabular-nums tracking-tighter leading-none italic">{stat.value}</h3>
                </div>
                <div className="mt-12 pt-10 border-t border-black/5 flex items-center justify-between">
                  ...
                  <div className="w-12 h-12 rounded-[1rem] bg-black/5 flex items-center justify-center text-slate-700 group-hover:bg-primary group-hover:text-slate-900 transition-all duration-500">
      ...
      <div className="grid lg:grid-cols-2 gap-12">
        {/* Recent Activity */}
        <Card className="bg-black/[0.02] border-black/10 backdrop-blur-3xl rounded-[3.5rem] overflow-hidden">
          <CardHeader className="p-12 border-b border-black/5">
            <div className="flex items-center justify-between">
               <CardTitle className="text-slate-900 text-2xl font-black uppercase tracking-tighter flex items-center gap-5 italic">
            ...
            <div className="divide-y divide-white/5">
              {recentProducts?.map((product) => (
                <div key={product.id} className="p-10 flex items-center gap-8 hover:bg-white/[0.04] transition-all group cursor-default">
                  <div className="w-16 h-16 rounded-[1.25rem] bg-primary/10 border border-primary/20 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-slate-900 transition-all duration-500 shadow-xl">
                    <Package className="w-7 h-7" />
                  </div>
                  <div className="flex-1 min-w-0 space-y-1">
                    <p className="text-[10px] font-black text-primary uppercase tracking-[0.4em] italic">INVENTORY INJECTED</p>
                    <p className="text-sm text-slate-900 font-black uppercase tracking-tight truncate italic">{product.title}</p>
                  </div>
              ...
              {recentMessages?.map((msg) => (
                <div key={msg.id} className="p-10 flex items-center gap-8 hover:bg-white/[0.04] transition-all group cursor-default">
                  <div className="w-16 h-16 rounded-[1.25rem] bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center text-cyan-500 group-hover:bg-cyan-500 group-hover:text-slate-900 transition-all duration-500 shadow-xl">
                    <MessageSquare className="w-7 h-7" />
                  </div>
                  <div className="flex-1 min-w-0 space-y-1">
                    <p className="text-[10px] font-black text-cyan-500 uppercase tracking-[0.4em] italic">INTEL RECEIVED</p>
                    <p className="text-sm text-slate-900 font-black uppercase tracking-tight truncate italic">FROM: {msg.name}</p>
                  </div>
                  <div className="text-right">
              ...
              {[
                ...
                <Link key={item.label} href={item.href}>
                  <Button variant="outline" className={cn(
                    "w-full h-44 flex-col gap-6 border-black/5 bg-black/[0.03] text-slate-600 transition-all duration-500 rounded-[2.5rem] font-black uppercase tracking-[0.2em] group italic",
                    item.bg, item.color
                  )}>
