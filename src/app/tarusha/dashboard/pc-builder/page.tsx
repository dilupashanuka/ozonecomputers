...
import { Cpu, Trash2, Plus, Image as ImageIcon, Eye, EyeOff, Save, FileText } from 'lucide-react';
import { addPCSlide, deletePCSlide, togglePCSlide, updatePCSlide, updatePCBuilderGlobalText } from './actions';
            ...
            {slides?.map((slide) => (
              <Card key={slide.id} className="group relative aspect-video glass rounded-[2.5rem] border-white/5 overflow-hidden transition-all hover:border-primary/50">
                ...
                <div className="absolute top-4 right-4 flex gap-2">
                  <form action={togglePCSlide}>
                    <input type="hidden" name="id" value={slide.id} />
                    <input type="hidden" name="currentStatus" value={String(slide.is_active)} />
                    <Button 
                      variant="ghost" 
                      size="icon" 
                      type="submit" 
