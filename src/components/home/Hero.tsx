...
export function Hero({ slides = [], settings }: HeroProps) {
  ...
  return (
    <section className="relative h-screen min-h-[800px] w-full overflow-hidden bg-slate-50">
      {/* Background Slides */}
      ...
      <div className="container relative z-10 mx-auto flex h-full flex-col justify-center px-6 pt-40 pb-24">
        <div className="max-w-5xl space-y-12">
          <div className="flex flex-wrap gap-4 animate-in fade-in slide-in-from-left-8 duration-700">
             ...
             <div className="inline-flex items-center gap-4 px-6 py-2.5 rounded-full bg-black/5 border border-black/10 backdrop-blur-2xl">
          ...
          <h1 className="text-6xl sm:text-7xl md:text-8xl lg:text-9xl font-black tracking-tighter text-slate-900 animate-in fade-in slide-in-from-bottom-12 duration-1000 leading-[0.85] uppercase italic">
          ...
          <div className="flex flex-wrap gap-6 md:gap-10 animate-in fade-in slide-in-from-bottom-12 duration-1000 delay-300 pt-8">
            <Link
              href="/products"
              className="group h-20 md:h-24 inline-flex items-center gap-5 rounded-2xl bg-primary px-12 md:px-16 text-xs md:text-sm font-black text-slate-900 uppercase tracking-[0.3em] hover:shadow-[0_20px_60px_rgba(239,68,68,0.6)] transition-all transform hover:-translate-y-2 italic shadow-2xl"
            >
              INITIALIZE SCAN
            ...
            <Link
              href="/pc-builder"
              className="h-20 md:h-24 inline-flex items-center gap-5 rounded-2xl border border-black/10 bg-black/5 px-12 md:px-16 text-xs md:text-sm font-black text-slate-900 uppercase tracking-[0.3em] backdrop-blur-2xl hover:bg-black/10 transition-all transform hover:-translate-y-2 italic shadow-2xl"
            >
      ...
      <div className="absolute right-12 bottom-32 hidden xl:flex flex-col gap-8 z-20 animate-in fade-in slide-in-from-right-12 duration-1000 delay-500">
         <div className="p-8 rounded-[2.5rem] bg-black/[0.03] border border-black/10 backdrop-blur-3xl space-y-6 shadow-2xl transform hover:scale-105 transition-transform">
            ...
            <div className="space-y-2">
               <p className="text-3xl font-black text-slate-900 uppercase italic tracking-tighter">FLAGSHIP</p>
               <p className="text-[10px] font-black text-slate-700 uppercase tracking-[0.3em] italic">RTX 4090 POWERED</p>
            </div>
         </div>

         <div className="p-8 rounded-[2.5rem] bg-black/[0.03] border border-black/10 backdrop-blur-3xl space-y-6 shadow-2xl transform translate-x-20 hover:scale-105 transition-transform">
            <div className="flex items-center justify-between gap-12">
               <span className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] italic">NETWORK</span>
               <Zap className="w-4 h-4 text-primary animate-pulse" />
            </div>
            <div className="space-y-2">
               <p className="text-3xl font-black text-slate-900 uppercase italic tracking-tighter">BRANCHES</p>
          ...
          {displaySlides.map((_, i) => (
            <button
              ...
              className={cn(
                ...
                i === currentSlide ? "h-16 bg-primary shadow-[0_0_15px_rgba(239,68,68,0.8)]" : "h-6 bg-black/10 hover:bg-white/30"
              )}
            />
          ))}
        </div>
