...
export function Navbar({ settings }: NavbarProps) {
  ...
  return (
    <>
      {/* Cinematic Top Bar */}
      <div className="fixed top-0 left-0 right-0 z-[1001] h-10 bg-[#050811] border-b border-white/5 hidden md:flex items-center overflow-hidden">
        <div className="container mx-auto px-6 flex justify-between items-center h-full">
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-3">
              <div className="w-2 h-2 rounded-full bg-primary animate-pulse" />
              <span className="text-[9px] font-black text-white uppercase tracking-[0.4em] italic leading-none">OZONE LABS OPERATIONAL</span>
            </div>
      ...
      <nav className={cn(
        ...
        scrolled ? "top-0 h-24 bg-[#050811]/90 backdrop-blur-2xl border-b border-white/10" : "top-0 md:top-10 h-28 bg-transparent",
        !visible && !isMenuOpen ? "-translate-y-full" : "translate-y-0"
      )}>
      <div className="container mx-auto px-6 h-full flex items-center justify-between">
        {/* Logo Section */}
        <Link href="/" className="flex items-center gap-6 group">
          <div className="relative h-14 w-14 sm:h-16 sm:w-16 bg-white/[0.03] rounded-[1.25rem] p-3 border border-white/10 group-hover:border-primary/50 transition-all duration-500 group-hover:shadow-[0_0_30px_rgba(239,68,68,0.3)] transform group-hover:rotate-6">
            <Image 
          ...
          <div className="flex flex-col flex-shrink-0">
            <span className="text-xl lg:text-2xl xl:text-3xl font-black tracking-tighter text-white leading-none group-hover:text-primary transition-colors italic uppercase">
              OZONE <span className="text-primary italic">LABS</span>
            </span>
            <span className="text-[8px] font-black text-slate-600 uppercase tracking-[0.5em] mt-1 group-hover:text-white transition-colors italic">ENGINEERING INTEGRITY</span>
          </div>
        </Link>
        ...
        <div className="hidden lg:flex items-center gap-6 xl:gap-10">
          {NAV_LINKS.map((link) => (
            <div key={link.label} className="relative group/item py-4">
              <Link 
                href={link.href} 
                className="text-[10px] xl:text-[12px] font-black uppercase tracking-[0.3em] text-slate-500 hover:text-white transition-all flex items-center gap-2 italic"
              >
                {link.label}
              ...
              {link.submenu && (
                <div className="absolute top-full left-0 w-72 pt-6 opacity-0 invisible group-hover/item:opacity-100 group-hover/item:visible transition-all duration-500 transform translate-y-4 group-hover/item:translate-y-0">
                  <div className="bg-[#0a0d14] border border-white/10 rounded-[2.5rem] p-4 shadow-[0_50px_100px_rgba(0,0,0,0.9)] overflow-hidden">
                    <div className="grid gap-2">
                      {link.submenu.map((sub) => (
                        <Link 
                          key={sub.label}
                          href={sub.href}
                          className="flex items-center gap-5 p-5 rounded-2xl hover:bg-white/[0.03] text-slate-400 hover:text-white transition-all group/sub"
                        >
                          <div className="w-12 h-12 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center group-hover/sub:bg-primary group-hover/sub:border-primary group-hover/sub:text-white transition-all duration-500 shadow-xl group-hover/sub:shadow-primary/20">
        ...
        <div className="flex items-center gap-3 lg:gap-4 xl:gap-8">
          <button 
            ...
            className="w-12 h-12 rounded-full flex items-center justify-center text-slate-500 hover:text-white hover:bg-white/5 transition-all duration-500"
          >
          ...
          <Link 
            ...
            className={cn(
              ...
              "hidden lg:flex h-12 xl:h-14 px-8 xl:px-10 text-[10px] xl:text-xs bg-primary hover:bg-primary/90 text-white font-black uppercase tracking-[0.3em] rounded-2xl shadow-[0_20px_40px_rgba(239,68,68,0.3)] transition-all hover:scale-105 active:scale-95 italic"
            )}
          >
          ...
          {/* Mobile Menu Toggle */}
          <button
            className="lg:hidden w-14 h-14 bg-white/5 rounded-2xl flex items-center justify-center text-white border border-white/10"
      ...
      <div className={cn(
        "fixed inset-0 bg-[#050811] lg:hidden transition-all duration-700 ease-in-out z-[9999] overflow-hidden",
        ...
        <div className="flex flex-col h-full pt-28 px-8 pb-12 relative z-10 overflow-y-auto custom-scrollbar">
          ...
          <div className="flex items-center justify-between mb-16">
            <div className="flex items-center gap-5">
              <div className="w-16 h-16 rounded-[1.5rem] bg-white/[0.03] border border-white/10 flex items-center justify-center shadow-2xl">
                 <Image src={settings?.logo_url || "/logo.png"} alt="Logo" width={40} height={40} className="object-contain" />
              </div>
              <div className="space-y-1">
                <span className="text-3xl font-black text-white tracking-tighter uppercase italic leading-none">OZONE <span className="text-primary">LABS</span></span>
                <div className="flex items-center gap-3">
            ...
            <button 
              ...
              className="w-16 h-16 rounded-2xl bg-white/5 border border-white/10 flex items-center justify-center text-white active:scale-90 transition-all duration-500"
            >
              <X className="w-8 h-8" />
            </button>
          </div>
          ...
          <div className="grid gap-6">
            {NAV_LINKS.map((link, i) => (
              <Link 
                key={link.label}
                href={link.href}
                className="group relative flex items-center justify-between p-10 rounded-[3rem] bg-white/[0.02] border border-white/5 active:bg-primary/10 active:border-primary/50 transition-all duration-500 overflow-hidden shadow-xl"
                style={{ transitionDelay: `${i * 100}ms` }}
                onClick={() => setIsMenuOpen(false)}
              >
                <div className="flex items-center gap-8">
                   <div className="w-16 h-16 rounded-[1.25rem] bg-white/[0.03] flex items-center justify-center group-active:scale-110 transition-transform shadow-2xl">
                      <ChevronRight className="w-8 h-8 text-primary" />
                   </div>
                   <div className="flex flex-col space-y-1">
                     <span className="text-4xl font-black text-white tracking-tighter uppercase leading-none italic">
                       {link.label}
                     </span>
                     <span className="text-[10px] font-black text-slate-600 uppercase tracking-[0.4em] italic">INITIALIZE SECTOR ACCESS</span>
                   </div>
                </div>
                <ArrowRight className="w-8 h-8 text-slate-800 group-active:text-white transition-colors" />
              </Link>
            ))}
            ...
            >
              <div className="flex items-center gap-8">
                 <div className="w-16 h-16 rounded-[1.25rem] bg-primary text-white flex items-center justify-center shadow-2xl shadow-primary/40">
          ...
          <div className="mt-16 pt-16 border-t border-white/10">
            <div className="space-y-10">
              ...
              <div className="grid gap-6">
                {BRANCHES.map((branch) => (
                  <div key={branch.name} className="p-10 rounded-[2.5rem] bg-white/[0.02] border border-white/5 flex items-center justify-between group shadow-xl">
                    <div className="flex flex-col space-y-2">
                      <span className="text-[10px] font-black text-slate-600 uppercase tracking-[0.4em] italic leading-none">NODE</span>
                      <span className="text-2xl font-black text-white uppercase tracking-tighter italic">{branch.name}</span>
                    </div>
                    <div className="flex flex-col text-right space-y-2">
                      ...
                      <span className="text-xl font-black text-primary group-active:text-white transition-colors italic tracking-widest">
                         {branch.phone}
                      </span>
                    </div>
                  </div>
