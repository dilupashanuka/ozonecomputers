...
export function MobileFilter({ currentCategory, categories, availableSpecs, availableBrands }: { 
  ...
  return (
    <Sheet>
      <SheetTrigger render={
        <div className="lg:hidden flex items-center gap-4 px-10 py-5 rounded-2xl bg-black/[0.03] border border-black/5 text-[11px] font-black uppercase tracking-[0.4em] text-slate-900 cursor-pointer active:scale-95 transition-all italic shadow-2xl backdrop-blur-3xl group">
          <SlidersHorizontal className="w-5 h-5 text-primary animate-pulse group-hover:scale-110 transition-transform" />
          FILTER_LAB_NODE
        </div>
      } />
      <SheetContent side="left" className="w-[340px] p-0 bg-slate-50/95 border-black/10 overflow-y-auto custom-scrollbar backdrop-blur-3xl">
        <div className="p-10">
          <SheetHeader className="mb-16">
            ...
            <SheetTitle className="text-4xl font-black text-slate-900 tracking-tighter uppercase italic leading-none drop-shadow-2xl">LAB <span className="text-primary italic">FILTERS</span></SheetTitle>
